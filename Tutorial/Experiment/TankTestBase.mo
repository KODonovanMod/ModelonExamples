...
model TankTestBase
    .Modelon.ThermoFluid.Volumes.TwoPhaseTankSimpleWall storageTank(enable_externalHeatPort = true,N_vaporPort = 1,N_liquidPort = 1,redeclare replaceable package Medium = .VaporCycle.Media.Naturals.HydrogenMbwr,redeclare replaceable .Modelon.ThermoFluid.Volumes.SubComponents.TankShapes.HorizontalCylinderEllipsoid geo(r_internal = r_inner,s_end = r_inner,L = L_cylinder,t_wall = t_insulation),redeclare replaceable .Modelon.ThermoFluid.Volumes.HeatTransfer.ConstantCoefficient internalConvection(alpha_dry_prescribed = fill(alpha_wall,2),alpha_wet_prescribed = fill(alpha_wall,2)),K_cond_surface = 1e-3,g_external = k_insulation / t_insulation,k_wall = k_insulation,p_start = p_start,V_liq_start = V_liq_start,vaporLiquidHeatTransfer(alpha = 4),tankInitQuantity = Modelon.ThermoFluid.Choices.TwoPhaseTankInitTypes.initFromMass,M_tot_start = massHydAvail) annotation(Placement(transformation(extent = {{-30.39814604388812,6.0},{-10.398146043888119,26.0}},origin = {0.0,0.0},rotation = 0.0)));
...
equation
    connect(checkValve.portA,storageTank.vaporPort[1]) annotation(Line(points = {{-8,39},{-20.39814604388812,39},{-20.39814604388812,25}},color = {255,128,0}));
    connect(checkValve.portB,Atmosphere.port[1]) annotation(Line(points = {{12,39},{21,39},{21,42},{28,42}},color = {255,128,0}));
    connect(storageTank.liquidPort[1],massFlowBoundary.port) annotation(Line(points = {{-20.39814604388812,7},{-20.39814604388812,-26},{-30.2,-26}},color = {0,190,0}));
    connect(storageTank.heat_external,ambient_T.port[1]) annotation(Line(points = {{-29.098146043888118,16.1},{-62,16.1},{-62,10}},color = {191,0,0}));
